...
## 使用说明
左上方为数据来源选择与开始/停止下载控制按钮  
右侧为筛选选项，筛选数据来源中的歌曲  
右下角为本地歌曲目录，可空  
左下角为保存方式（如果下载歌曲，跳过已有目录存在时将直接跳过，本地目录中存在时将直接复制）  

**设置数据来源 → 筛选方式 → 保存方式，点击开始即可下载。**  
...
```
BeatSpider.exe -config "360度歌曲包" -minimize -start -unzip -exit
```
我推荐的本地曲包自动更新设置方案：  
...
3. BeatSpider设置好筛选选项，自己设一个配置文件名称，点击保存  
4. 想好自己打算使用的参数，建议按刚才的示例设置，刚才的示例参数为 `-config "配置名称" -minimize -start -unzip -exit` 也可以简写为 `-c "配置名称" -m -s -u -e`  
5. 打开`任务计划程序`（可在Win10左下角自带的搜索找到）  
6. 点击左上角的 操作 → 创建基本任务（如果没有创建基本任务，请先点击左侧空白处再重新点击操作）  
7. 名称和描述：自己看着设  
8. 触发方式：自己看着设  
9. 操作：启动程序  
10. 程序那一栏点击`浏览`，然后找到本软件（BeatSpider.exe）  
11. 添加参数（如果是刚才的示例，则填写`-config "360度歌曲包" -minimize -start -unzip -exit` 或 `-c "360度歌曲包" -m -s -u -e`）  
